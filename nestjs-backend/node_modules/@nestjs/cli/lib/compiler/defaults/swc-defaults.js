...
export const swcDefaultsFactory = (tsOptions, configuration, tsConfigExclude = []) => {
    ...
    return {
        ...
        cliOptions: {
            ...
            quiet: false,
            watch: false,
            stripLeadingPaths: shouldStripLeadingPaths(tsOptions?.rootDir, configuration?.sourceRoot ?? 'src'),
            ...builderOptions,
        },
    };
};
 ...
 */
export function resolveSwcModuleType(tsOptions, esmPackage = isEsmProject()) {
    if (!esmPackage) {
        return 'commonjs';
    }
 ...
 */
function resolveBaseUrl(tsOptions) {
 ...
 */
function shouldStripLeadingPaths(rootDir, sourceRoot) {
    if (!rootDir) {
        return true;
    }
    ...
    const normalizedSourceRoot = convertPath(sourceRoot)
        .replace(/^\.\//, '')
        .replace(/\/+$/, '');
    return (normalizedRootDir === normalizedSourceRoot ||
